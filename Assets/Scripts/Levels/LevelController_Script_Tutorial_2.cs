		...
		{
			{
				...
				complete = new bool[4];
				processors[0] = createProcessor(processorAllyPrefab, 9, 1, -1, 0, 0, 1);
				processors[1] = createProcessor(processorEnemyPrefab, 4, -1, 5, 0, 0, 1);
				processors[2] = createProcessor(processorNuetralPrefab, 0, 0, 2, 0, 2, 1);
				processors[3] = createProcessor(processorNuetralPrefab, 0, 0, 2, 0, -3, 1);
				gameMasterV2_Script.instance.setPowerSupplyAlly(createProcessor(powerSupplyPrefab, 0, 1, -3, 0, 0, 0));
				gameMasterV2_Script.instance.setPowerSupplyEnemy(createProcessor(powerSupplyPrefab, 0, -1, 7, 0, 0, 0));
